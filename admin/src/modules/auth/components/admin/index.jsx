...
const Admin = ({ location, history }) => {
  ...
  const createAdminUser = (formValue) => {
    firestore.collection("admin").add({
      ...
      .then(() => createAdsDumData())
      .then(() => createPackagesDumData())
      .then(() => {
  ...
  const createPackagesDumData = () => {
    ...
    const arr = [
      ...
      {
        duration: "Monthly",
        packageId: "gold_monthly",
        packageName: "Gold Status Monthly",
        packagePricing: "9.99",
        platform: "android"
      },
      {
    ...
    arr.forEach((doc) => {
      const docRef = firestore.collection("packages").doc();
      batch.set(docRef, doc);
    });
